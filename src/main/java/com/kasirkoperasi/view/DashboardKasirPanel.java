...
public class DashboardKasirPanel extends JPanel {
    public DashboardKasirPanel() {
        setLayout(new BorderLayout());
        setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20));

        JLabel title = new JLabel("Dashboard Kasir", SwingConstants.LEFT);
        ...
        cards.add(createCard("Transaksi Hari Ini", getSingleValue("SELECT COUNT(*) FROM transaksi WHERE DATE(tanggal)=CURDATE()")));
        cards.add(createCard("Pendapatan Hari Ini", "Rp " + getFormattedValue("SELECT COALESCE(SUM(total),0) FROM transaksi WHERE DATE(tanggal)=CURDATE()")));
